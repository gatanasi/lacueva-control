...
public class CommonsController {
	...
	logger.info("Changing active shop to shopId= " + id);

	Shop newShop = shopDao.find(id);
	model.addAttribute("currShop", newShop);
	return newShop.getShopName();
    }
	...
	for (GrantedAuthority grantedAuthority : authorities) {
	    String role = grantedAuthority.getAuthority().substring(5);
	    if ("ADMIN".equals(role)) {
		shopList = shopDao.getAll();
		currShop = shopList.get(0);
		break;
	    } else {
		if (isNumeric(role)) {
		    Long shopId = Long.parseLong(role);
		    Shop foundShop = shopDao.find(shopId);
		    shopList.add(foundShop);
		    currShop = foundShop;
		}
	    }
	}
	...
	model.addAttribute("currShop", currShop);
	return "redirect:/";
    }
